...
const PROJECTS = [
	{ path: 'applications/server/package.json', out: 'SecGate' },
...
for (const [OS, PKG_KEY] of Object.entries(OS_ARCH)) {
	...
	for (const project of PROJECTS) {
		...
		await pkgExec(
			[project.path, '-t', `node${NODE_VER}-${PKG_KEY.platform}-${PKG_KEY.arch}`,  '-o', resolve(releaseDir, `SecGate-${OS}`, project.out)]
		);
	}
}
