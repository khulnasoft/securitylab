import { Campaign, ParsingStatus, Server } from '@secgate/models';
import { getMigratedCampaignORM } from '@secgate/migrations';
...
export async function importCampaign(
	...
	try {
		...
		const fileName = path.join(getDbPath(context.config.databaseMode), 'campaign', campaignId, 'db.secgate');
