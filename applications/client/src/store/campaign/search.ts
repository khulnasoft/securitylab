import { createSorter } from '@secgate/client/components/create-sorter';
import type { AnyModel, FieldToNames, ProcessedSearchItem, SearchItem } from '@secgate/client/types/search';
import { SearchFilterOptions, SearchSortOptions } from '@secgate/client/types/search';
...
export class SearchStore extends ExtendedModel(() => ({
	baseModel: modelClass<SecGateModel>(SecGateModel),
	props: {
	...
	*runSearch(e?: FormEvent<HTMLFormElement>) {
	...
	static getRankedValues(item: AnyModel): (string | number | null | undefined)[] {
		if (item instanceof OperatorModel) {
			...
			return [item.computedName, item.name];
