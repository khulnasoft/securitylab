...
import { CarbonIcon, DialogEx, HoverButton } from '@secgate/client/components';
import { DialogBodyEx } from '@secgate/client/components/Dialogs/DialogBodyEx';
import { DialogFooterEx } from '@secgate/client/components/Dialogs/DialogFooterEx';
import { createState } from '@secgate/client/components/mobx-create-state';
import type { AnonymizationInput, CampaignModel, FindReplaceInput } from '@secgate/client/store';
			...
			try {
				...
				if (res.status === 200) {
					try {
						...
						link.setAttribute('download', `${campaign.name}.secgate`);
