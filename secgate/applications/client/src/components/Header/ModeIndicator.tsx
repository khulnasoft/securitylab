...
import type { PopoverButtonProps } from '@secgate/ui-styles';
import { Flex, ExternalLink, CoreTokens, PopoverButton, Txt } from '@secgate/ui-styles';
...
export const ModeIndicator = observer<Omit<PopoverButtonProps, 'content'> & { fullName?: boolean }>(
	({ popoverProps, fullName, ...props }) => {
		...
		let team = 'Blue Team';
		let description = 'Restricted SecGate functionality. Editing and commenting features are not available.';
		let teamCSS = blueStyle;

		if (isRedTeam) {
			teamAcronym = fullName ? 'Red Team' : 'RT';
			team = 'Red Team';
			description = 'Full SecGate functionality. Editing and commenting is not restricted.';
			teamCSS = redStyle;
		}
			...
			<PopoverButton
				...
				content={
					<Flex column gap={4} align="start" css={{ width: 320, padding: 12 }}>
						...
						<ExternalLink href="https://github.com/khulnasoft/securitylab#red-team--blue-team-modes">
							Learn more
						</ExternalLink>
					</Flex>
				}
