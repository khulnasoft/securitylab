...
const serverStartLogs = async (ctx: ServerMachineContext, clientUrl?: string): Promise<void> => {
	...
	const helpLink = 'https://github.com/khulnasoft/securitylab#readme';
	const mode = !ctx.config.redTeam ? `${cf.blueBg} BLUE TEAM ${cf.reset}` : `${cf.redBg} RED TEAM ${cf.reset}`;

	logLine.push(
		`  ${cf.bold}${cf.white}SecGate Server${cf.reset} ${ver}${cf.reset}`,
		`  SecGate Client ${cf.blue}${cf.underlined}${usedClientUrl}${cf.reset}`,
